...
class TestShimMock(unittest.TestCase):
    def setUp(self):
        uninstall_shim()
        self.mock_tracer_provider = MagicMock(spec=trace.TracerProvider)
        install_shim(self.mock_tracer_provider)

    def tearDown(self):
        uninstall_shim()

    def test_shim_span_contextmanager_calls_end_once(self):
        # This was a bug in first implementation where the underlying OTel span.end() was
        # called twice
        oc_tracer = OcTracer()
        with oc_tracer.start_span("foo"):
            pass

        mock_span = self.mock_tracer_provider.get_tracer().start_span()
        mock_span.end.assert_called_once

    def test_shim_span_finish_calls_end_once(self):
        oc_tracer = OcTracer()
